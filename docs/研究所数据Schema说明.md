...
- `dev-baseline-20260902` 是统一版本管理建立后的初始开发基线。
- `dev-v1.1-single-section` 表示研究所正式单截面DATABASE完成适配并通过完整回归测试。
- 该版本不包含四截面训练或预测；INLET/OUTLET可识别，但暂不参与代理模型。
- Legacy Validation Schema继续保留，用于旧代理模型与数据链路回归验证。
- 封板验证中临时使用单个线性模型运行完整1D/2D/3D训练链路，只用于确认数据读取、
  分区、拟合和模型保存流程可以运行，不构成代理模型选择或预测精度验证。
