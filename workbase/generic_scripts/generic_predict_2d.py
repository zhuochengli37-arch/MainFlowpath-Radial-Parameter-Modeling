...
def main() -> None:
    ...
    logger = setup_logger("generic_predict_2d", log_dir=config.log_dir)

    parser = argparse.ArgumentParser(description="通用 2D 表格预测")
    ...
    input_path = args.input or config.predict_input_path(2)
    model_output = args.model_output or config.predict_model_output_dir(2)
    output_dir = args.output or config.predict_output_dir(2)

    logger.info("=" * 70)
    logger.info("通用 2D 预测开始")
    logger.info("=" * 70)
    logger.info("配置参数:")
