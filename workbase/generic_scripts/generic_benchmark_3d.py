...
def main() -> None:
    ...
    logger = setup_logger("generic_benchmark_3d", log_dir=config.log_dir)

    parser = argparse.ArgumentParser(description="通用 3D 表格 benchmark")
    ...
    input_path = args.input or config.benchmark_input_path(3)
    output_dir = args.output or config.benchmark_output_dir(3)
    max_samples = args.max_samples if args.max_samples is not None else config.benchmark_max_samples(3)
    n_splits = args.n_splits if args.n_splits is not None else config.benchmark_n_splits(3)
    include_gpr = args.include_gpr or config.benchmark_include_gpr(3)

    logger.info("=" * 70)
    logger.info("通用 3D benchmark 开始")
    logger.info("=" * 70)
    logger.info("配置参数:")
    ...
    try:
        ...
        logger.info("=" * 70)
        logger.info("[OK] 通用 3D benchmark 完成")
        logger.info("=" * 70)
        ...
        logger.error("=" * 70)
        logger.error("[ERROR] 通用 3D benchmark 失败")
        logger.error("=" * 70)
