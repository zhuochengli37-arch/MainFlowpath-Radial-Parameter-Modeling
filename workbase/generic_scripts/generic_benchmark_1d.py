...
def main() -> None:
    ...
    logger = setup_logger("generic_benchmark_1d", log_dir=config.log_dir)

    parser = argparse.ArgumentParser(description="通用 1D 表格 benchmark")
    ...
    input_path = args.input or config.benchmark_input_path(1)
    output_dir = args.output or config.benchmark_output_dir(1)
    max_samples = args.max_samples if args.max_samples is not None else config.benchmark_max_samples(1)
    n_splits = args.n_splits if args.n_splits is not None else config.benchmark_n_splits(1)
    include_gpr = args.include_gpr or config.benchmark_include_gpr(1)

    logger.info("=" * 70)
    logger.info("通用 1D benchmark 开始")
    logger.info("=" * 70)
    logger.info("配置参数:")
    ...
    try:
        ...
        logger.info("=" * 70)
        logger.info("[OK] 通用 1D benchmark 完成")
        logger.info("=" * 70)
        ...
        logger.error("=" * 70)
        logger.error("[ERROR] 通用 1D benchmark 失败")
        logger.error("=" * 70)
